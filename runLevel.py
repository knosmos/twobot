...
class player():
    def __init__(self,x,y,z,h=0,short=True):
        ...
        world.attach(self.frontNode)
        self.nodePath.attachNewNode(self.frontNode)
        NodePath(self.frontNode).setY(-0.5)
        ...
        NodePath(self.backNode).setY(0.5)
...
def keyboardControl():
    ...
    for i in level:
        if i.type == 'player':
            ...
            if (forward and 'block' in [n.name for n in i.frontNode.getOverlappingNodes()]) or (not(forward) and 'block' in [n.name for n in i.backNode.getOverlappingNodes()]):
                speed.setY(speed.getY()/1.5)
